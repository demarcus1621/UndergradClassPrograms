   ...
   /**
    ...
    public static class TrieAutocomplete implements Autocompletor {
        ...
        private void add(String word, double weight) {
            // TODO: Implement add
            int size = 0;
            char currentChar = word.charAt(size);
            Node currentNode = myRoot;
            Node x;
            while(size < word.length()){
               currentChar = word.charAt(size);
               if( word == null || word.equals(null) )
                  throw new NullPointerException();
               if( weight < 0 )
                  throw new IllegalArgumentException();
               if( currentNode.getChild(currentChar) == null && weight > currentNode.getWeight() ){
                  x = new Node(currentChar, currentNode, weight);
                  currentNode.children.put(currentChar, x);
                  currentNode = x;
               }
               else if( currentNode.getChild(currentChar) == null ){
                  x = new Node( currentChar, currentNode, currentNode.getWeight() );
                  currentNode.children.put(currentChar, x);
                  currentNode = x;
               } 
               else {
        ...
        public Iterable<String> topMatches(String prefix, int k) {
            ...
            if( prefix == null )
               throw new NullPointerException();
            Node CurrentRoot = myRoot;
            PriorityQueue<Node> returnable = new PriorityQueue<Node>();
            for( int x = 0 ; x < prefix.length() ; x++ ) {
               CurrentRoot = CurrentRoot.getChild( prefix.charAt(x) );
               if( CurrentRoot == null )
                  return new ArrayList<String>();
            }
            PriorityQueue<Node> visit = new PriorityQueue<Node>(CurrentRoot.children.values());
            while( visit.size() > 0 ){
               Node temp = visit.poll();
               for( Node x : temp.children.values() ){
                  if( x.isWord ){
                     if( returnable.size() < k )
                        returnable.add(x);
                     else if( returnable.peek().compareTo(x) < 0 ){
                        returnable.poll();
                        returnable.add(x);
                     }
                  }
                  if( x.children.size() > 0 )
                     visit.addAll(x.children.values());
               }
            }
            ArrayList<String> finish = new ArrayList<String>();
            for( Node x : returnable )
               finish.add(0,x.getWord());
            return finish;
        }
